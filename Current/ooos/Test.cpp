...
void KernelEntry(void)
{
	...
	CurrentProcess = &KNode;
	CurrentProcess->Proc = &ker;
	...
	*/
	...
	CurrentProcess = ReadyProcesses->Pop_Front();
	//CurrentProcess->Proc = &InitProcess;
	CurrentProcess->Next = NULL;
	...
	// Call the first task properly, rest of the 
	// task switches are handled by the dispatcher (Timer ISR for now)

	// 0x50:00 = first process  (GDT[20]) /2 then << 3
	__asm
	{
		//STI
		;call [0x50:0]
